...
% Classify unknown audio files using multiple distance metrics
    ...
    for i = 1:num_unknown
        ...
        % Calculate similarities to class 1
        class1_similarities = zeros(size(class1_features, 1), 1);
        for j = 1:size(class1_features, 1)
            class1_similarities(j) = cosine_similarity(unknown_feature, class1_features(j, :));
        end
        
        % Calculate similarities to class 2
        class2_similarities = zeros(size(class2_features, 1), 1);
        for j = 1:size(class2_features, 1)
            class2_similarities(j) = cosine_similarity(unknown_feature, class2_features(j, :));
        end
        ...
        % Calculate distances to class 1
        class1_distances = zeros(size(class1_features, 1), 1);
        for j = 1:size(class1_features, 1)
            class1_distances(j) = euclidean_distance(unknown_feature, class1_features(j, :));
        end
        
        % Calculate distances to class 2
        class2_distances = zeros(size(class2_features, 1), 1);
        for j = 1:size(class2_features, 1)
            class2_distances(j) = euclidean_distance(unknown_feature, class2_features(j, :));
        end
        ...
        % Calculate distances to class 1
        class1_manhattan = zeros(size(class1_features, 1), 1);
        for j = 1:size(class1_features, 1)
            class1_manhattan(j) = manhattan_distance(unknown_feature, class1_features(j, :));
        end
        
        % Calculate distances to class 2
        class2_manhattan = zeros(size(class2_features, 1), 1);
        for j = 1:size(class2_features, 1)
            class2_manhattan(j) = manhattan_distance(unknown_feature, class2_features(j, :));
        end
